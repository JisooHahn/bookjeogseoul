const bookDetailLayout = (() => {
    ...
    const showThisBookPosts = async (bookPostListData) => {
        ...
        bookPostListData.forEach((post) => {
            ...
            text += `
                <li class="slide-item">
                    <a href="/post/bookpost/${post.id}" class="report-cont">
                        <p class="title">${post.bookPostTitle}</p>
                        <p class="cont">${post.bookPostText}</p>
                    </a>
                    <div class="profile">
                        <a href="/personal/mypage/${post.memberId}" class="inner gtm-report-lib">
                            <div class="image" style="background-image: url('${imageUrl}');"></div>
                            <p class="name">${post.memberNickName}</p>
                        </a>
                        <button style="display: none" type="button" class="gtm-report-follow" id="${post.memberId}">팔로우</button>
                    </div>
                </li>
            `;
        });
