...
public enum SponsorMemberStatus {
    ACTIVE("활성"),
    DORMANCY("휴면"),
    SUSPENDED("정지"),
    DELETE("탈퇴");
    ...
    private SponsorMemberStatus(String code){
        this.code = code;
    }
...
//    직접 작성한 code 혹은 label이 있을 경우 재정의해야한다.
    @JsonCreator
    public static SponsorMemberStatus forCode(String code){
//        화면에서 전달받은 문자열(code)을
        for(SponsorMemberStatus sponsorMemberStatus  : SponsorMemberStatus.values()){
//            전체 code와 비교하여 같다면
            if(sponsorMemberStatus.getCode().equals(code)){
//                해당 code의 memberType 객체를 리턴한다.
                return sponsorMemberStatus;
            }
        }
