...
//        log.info(commentList.toString());
    ...
    public String goToBookPost(HttpSession session, HttpServletRequest request) {
        SponsorMemberDTO sponsor = (SponsorMemberDTO) session.getAttribute("sponsorMember");

        if (sponsor != null) {
            session.setAttribute("alertMessage", "개인회원만 사용 가능한 서비스입니다.");

            // Referer 가져오기 (이전 페이지 URL)
            String referer = request.getHeader("Referer");
            return "redirect:" + (referer != null ? referer : "/main/main");
        }

        PersonalMemberDTO member = (PersonalMemberDTO) session.getAttribute("member");
        session.setAttribute("memberId", member.getId()); // 세션을 이용한 memberId 저장
...
//        log.info(tempList.toString());
    ...
    public ResponseEntity<byte[]> getProfileImage(@RequestParam("path") String path,
        ...
        if (!imageFile.exists()) {
            try {
                imageFile = new ClassPathResource("static/images/common/default-donate-image.png").getFile();
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
