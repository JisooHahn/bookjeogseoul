...
public interface PostService {
    ...
    // 기부글 VO를 DTO로 전환
    public default ReceiverDTO toReceiverDTO(ReceiverVO receiverVO) {
    ...
    // 기부인증글 VO를 DTO로 전환
    public default DonateCertDTO toDonateCertDTO(DonateCertVO donateCertVO){
        DonateCertDTO donateCertDTO = new DonateCertDTO();
        if (donateCertVO != null) {
            donateCertDTO.setId(donateCertVO.getId());
            donateCertDTO.setDonateCertTitle(donateCertVO.getDonateCertTitle());
            donateCertDTO.setDonateCertText(donateCertVO.getDonateCertText());
        }
        return donateCertDTO;
    }

    // 내가 작성한 전체 기부글 목록 조회
    List<ReceiverPostDTO> getMyAllReceiverPosts(Long memberId);

    // 내가 작성한 전체 기부 인증글 목록 조회
    List<DonateCertPostDTO> getMyAllDonateCerts(Long memberId);
