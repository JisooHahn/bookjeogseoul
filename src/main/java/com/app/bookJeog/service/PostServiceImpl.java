...
//        log.info(donateCertPostDTOList.toString());
    ...
    public void deleteBookPost(Long bookPostId) {
        postDAO.deletePostAlarm(bookPostId);
        postDAO.deleteBookPostLike(bookPostId);
        postDAO.deleteBookPostFile(bookPostId);
    ...
    // 마이페이지용 기부글, 기부인증글 두개씩 조회
    @Override
    public Map<String, Object> getMyPageData(Long memberId) {
        Map<String, Object> resultMap = new HashMap<>();

        // 기부글 조회 및 썸네일
        List<ReceiverVO> receiverVOs = postDAO.findMyReceivers(memberId);
        System.out.println("ReceiverVOs: " + receiverVOs);

        List<FileVO> receiverFiles = receiverVOs.stream()
                .map(receiver -> fileDAO.findReceiverFileByPostId(receiver.getId()))
                .collect(Collectors.toList());
        System.out.println("ReceiverFiles: " + receiverFiles);

        // 기부인증글 조회 및 썸네일
        List<DonateCertVO> donateCertVOs = postDAO.findMyDonateCerts(memberId);
        System.out.println("DonateCertVOs: " + donateCertVOs);

        List<FileVO> donateCertFiles = donateCertVOs.stream()
                .map(donateCert -> fileDAO.findDonateCertFileByPostId(donateCert.getId()))
                .collect(Collectors.toList());
        System.out.println("DonateCertFiles: " + donateCertFiles);

        // VO -> DTO 변환
        List<ReceiverDTO> receiverDTOs = receiverVOs.stream()
                .map(this::toReceiverDTO)
                .collect(Collectors.toList());
        List<DonateCertDTO> donateCertDTOs = donateCertVOs.stream()
                .map(this::toDonateCertDTO)
                .collect(Collectors.toList());

        // 결과 맵에 담기
        resultMap.put("receivers", receiverDTOs);
        resultMap.put("receiverFiles", receiverFiles);
        resultMap.put("donateCerts", donateCertDTOs);
        resultMap.put("donateCertFiles", donateCertFiles);
    ...
    // 내가 쓴 전체 기부글
    @Override
    public List<ReceiverPostDTO> getMyAllReceiverPosts(Long memberId) {
        return postDAO.findMyAllReceiverPosts(memberId);
    }

    // 내가 쓴 전체 기부인증글
    @Override
    public List<DonateCertPostDTO> getMyAllDonateCerts(Long memberId) {
        return postDAO.findMyAllDonateCerts(memberId);
    }
    @Override
