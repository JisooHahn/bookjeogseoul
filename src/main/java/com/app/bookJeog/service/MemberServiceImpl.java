...
public class MemberServiceImpl implements MemberService {
    ...
    public PersonalMemberDTO checkEmail(PersonalMemberDTO personalMemberDTO) {
        ...
        log.info("personalMemberVO: {}", personalMemberVO);
    ...
    public void saveNewProfileImage(MultipartFile file, Long memberId) {
        ...
        try {
            ...
            if (!uploadDir.exists()) {
                ...
                log.info("디렉터리 생성 여부: {}", isCreated);
            }

            log.info("프로필 이미지 저장 경로: {}", rootPath);
            ...
            log.info("프로필 정보: {}", profileDTO);

        } catch (IOException e) {
            log.error("IOException 발생: {}", e.getMessage(), e);
            throw new RuntimeException("프로필 이미지 저장 실패", e);
        } catch (Exception e) {
            log.error("예기치 못한 에러 발생: {}", e.getMessage(), e);
